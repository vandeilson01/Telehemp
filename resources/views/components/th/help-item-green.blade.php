<article {{ $attributes->merge(['class' => "flex flex-row lg:flex-col items-center w-full lg:w-auto lg:h-full"]) }}>
    ...
    <div class="p-2 rounded-xl bg-gradient-to-br from-greenth via-darkgreenth to-darkgreenth rounded-sm space-x-2 lg:space-x-0 grid grid-flow-col lg:grid-flow-row lg:auto-rows-max w-full lg:h-full items-center divide-x lg:divide-none lg:justify-items-center">
        {{ $icon }}
        <div class="h-full items-center px-2 text-left lg:text-center">
            <p class="font-thunder text-xl uppercase text-white lg:border-t-2 lg:border-t-white lg:pt-2 lg:mt-3">{{ $text }}</p>
        </div>
    </div>
</article>
